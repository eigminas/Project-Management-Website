<?php
    ...
    if($id === -1 && $author === -1 && $project_id === -1) {
        ...
        $fields = explode('/', $str);
        $result = $note->getAllNotes($fields);
        if($result->rowCount() === 0) {
            $response['error'] = 'No Notes Found';
    ...
    // Case 2: get all notes by author
    if($id === -1 && $author !== -1 && $project_id === -1) {
        $str = substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], 'get.php') + strlen('get.php'));
        $str = substr($str, 1, strpos($str, '?') -2);
        $fields = explode('/', $str);
        $note->author = $author;
        $result = $note->getAllNotesByAuthor($fields);
        if($result->rowCount() === 0) {
            $response['error'] = 'No Notes Found';
            http_response_code(404);
            exit(json_encode($response));
    ...
    // Case 3: get all notes by project_id
    if($id === -1 && $author === -1 && $project_id !== -1) {
        $str = substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], 'get.php') + strlen('get.php'));
        $str = substr($str, 1, strpos($str, '?') -2);
        $fields = explode('/', $str);
        $note->project_id = $project_id;
        $result = $note->getAllNotesByProject($fields);
        if($result->rowCount() === 0) {
            $response['error'] = 'No Notes Found';
            http_response_code(404);
            exit(json_encode($response));
